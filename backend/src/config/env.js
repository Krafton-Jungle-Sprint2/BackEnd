...
try {
  ...
  console.log('ℹ️  .env 파일을 찾을 수 없습니다. Docker 환경 변수를 사용합니다.');
}
...
if (missingEnvVars.length > 0) {
  ...
  console.error('현재 환경 변수:', Object.keys(process.env).filter(key => key.includes('JWT') || key.includes('DATABASE')));
  process.exit(1);
}
...
function validateConfig() {
  ...
  console.log('📊 현재 설정:');
  console.log(`   - DATABASE_URL: ${config.database.url ? '✅ 설정됨' : '❌ 누락'}`);
  console.log(`   - JWT_SECRET: ${config.jwt.secret ? '✅ 설정됨' : '❌ 누락'}`);
  console.log(`   - REFRESH_TOKEN_SECRET: ${config.jwt.refreshSecret ? '✅ 설정됨' : '❌ 누락'}`);
  
  // JWT 시크릿 키 길이 검증
  if (config.jwt.secret && config.jwt.secret.length < 32) {
    console.warn('⚠️  JWT_SECRET이 너무 짧습니다. 보안을 위해 최소 32자 이상을 권장합니다.');
  }
  
  if (config.jwt.refreshSecret && config.jwt.refreshSecret.length < 32) {
    console.warn('⚠️  REFRESH_TOKEN_SECRET이 너무 짧습니다. 보안을 위해 최소 32자 이상을 권장합니다.');
  }
